		...
		if (new Random().nextInt(100) + 1 > 75) {
			...
			if (chance > 75) {
				lootlist.add(new Consumable("Potato",
						((int) (userHealth * .05)), "consumable", 1, 0, 0));
			} else if (chance > 50) {
				lootlist.add(new Consumable("Small HP Potion",
						((int) (userHealth * .1)), "consumable", 10, 0, 0));
			} else if (chance > 40) {
				lootlist.add(new Consumable("Fish", ((int) (userHealth * .15)),
						"consumable", 15, 0, 0));
			} else if (chance > 30) {
				lootlist.add(new Consumable("Chicken",
						((int) (userHealth * .2)), "consumable", 20, 0, 0));
			} else if (chance > 20) {
				lootlist.add(new Consumable("Turkey Leg",
						((int) (userHealth * .25)), "consumable", 25, 0, 0));
			} else if (chance > 10) {
				lootlist.add(new Consumable("Beef", ((int) (userHealth * .3)),
						"consumable", 30, 0, 0));
			} else if (chance > 2) {
				lootlist.add(new Consumable("Large HP Potion",
						((int) (userHealth * .5)), "consumable", 50, 0, 0));
			} else {

	...
	public static String fishing(User user) {
		...
		if (new Random().nextInt(100) + 1 > 75) {
			...
			if (user.getLevel("Fishing") < 5) {
				...
				if (chance <= 15) {
					user.itemList.add(new Cookable("Raw Swordfish", 400, 1,
							user, 5));
				...
				if (chance <= 15) {
					user.itemList.add(new Cookable("Raw Shark", 1000, 1, user,
							5));
					user.getLevelObject("Fishing").gainXp(1000);
					result = "You catch a lobster and gain 1000 XP!";
				} else if (chance <= 50) {
					user.getLevelObject("Fishing").gainXp(500);
					int coins = new Random().nextInt(100) + 1;
					user.setMoney(user.getMoney() + coins);
					...
					user.itemList.add(new Cookable("Raw Swordfish", 400, 1,
							user, 5));
					user.getLevelObject("Fishing").gainXp(400);
					result = "You catch a lobster and gain 400 XP!";
				}
			}
		} else {
			user.getLevelObject("Fishing").gainXp(10);

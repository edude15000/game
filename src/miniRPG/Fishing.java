	...
	public static String fishing(User user) {
		...
		if (new Random().nextInt(100) + 1 > 75) {
			...
			if (user.getLevel("Fishing") < 5) {
				...
				if (chance <= 25) {
					user.itemList.add(new Cookable("Raw Fish", 200, 5, user, 5));
				...
				if (chance <= 15) {
					user.itemList.add(new Cookable("Raw Lobster", 400, 10, user, 5));
				...
				if (chance <= 15) {
					user.itemList.add(new Cookable("Raw Swordfish", 750, 15, user, 5));
				...
				if (chance <= 15) {
					user.itemList.add(new Cookable("Raw Shark", 1500, 20, user, 5));
